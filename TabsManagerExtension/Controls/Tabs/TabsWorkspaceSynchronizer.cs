...
namespace TabsManagerExtension.Controls.Tabs {
    ...
    internal sealed class TabsWorkspaceSynchronizer {
        ...
        private readonly Action _stopFileMonitor;
        ...
        public TabsWorkspaceSynchronizer(
            ...
            _stopFileMonitor = stopFileMonitor;
        }
        ...
        public bool EnsureSolutionLoaded(string solutionName, Action ensureFileMonitor) {
        ...
        public void HandleDocumentOpened(EnvDTE.Document document) {
            ...
            var tabItemDocument = _tabCollectionManager.Find(document) ?? new TabItemDocument(document);
        ...
        public void HandleDocumentClosing(EnvDTE.Document document) {
            ...
            if (!this.IsSolutionClosing && !_closedTabsHistory.ConsumeClosingMark(tabItemDocument)) {
        ...
        public void HandleWindowClosing(EnvDTE.Window closingWindow) {
            ...
            VsixThreadHelper.RunOnUiThread(
                _dispatcher,
                () => _toolWindowSessionManager.Save(this.IsSolutionClosing),
        ...
        public void HandleSolutionClosing() {
            ...
            // VS не имеет отдельного события отмены закрытия solution. Проверяем результат
            // после завершения очереди UI и возвращаем синхронизатор в рабочее состояние.
        ...
        public void UpdateWindowTabsInfo() {
            ...
            _tabCollectionManager.ForEach<TabItemWindow>(tabItemWindow => {
                try {
        ...
        private void LoadOpenTabs() {
            ...
            foreach (EnvDTE.Document document in _dte.Documents) {
                _tabCollectionManager.AddAutomatically(new TabItemDocument(document));
            }
            foreach (EnvDTE.Window window in _dte.Windows) {
                ...
                if (shellWindow.IsTabWindow()) {
                    _tabCollectionManager.AddAutomatically(new TabItemWindow(shellWindow));
                }
            }
        ...
        public ClosedTabEntry CreateClosedTabEntry(TabItemBase tabItem) {
